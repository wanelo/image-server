...
exec bin/image-server --local_base_path ${BASE_PATH} --uploader ${UPLOADER} --aws_bucket ${AWS_S3_BUCKET} --aws_region ${AWS_REGION} --listen ${SERVER_LISTEN} --remote_base_url ${IMAGE_SERVER_REMOTE_BASE_URL} server